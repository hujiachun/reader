 ...
 * @author hujiachun
 */
public class ReaderSanityTestCase extends TestScript{

	private static List<String> pitchOnCategory;
	...
	public boolean isP1() {
		...
		try {
			...
			for (Method m : targeClass.getMethods()) {
				if (m.getName().equals(this.getName()) && m.getAnnotation(Priority.class).value() == 1) {
					return true;
				}
			}
	...
	protected void runTest() throws Throwable {
		...
		if (isMA01) {
			this.sleep(3000);
			this.yunosTest("start com.meizu.media.reader");
		}
		
		if(isMA01){
			if (this.getName().equals("test005AutoChangeBanner")){    //这里适配自己模块对应第一条不自杀进程的用例；
				Log.i("alitest", "reader.jar Test Start");    //使用android的Log，在logcat中打印。自行更改其jar包名称
				
			}
			else if(this.getName().equals("test078PersonalDataGet")){    //自行更改模块对应最后一条不自杀进程的用例；
				Log.i("alitest","reader.jar Test Done");    //自行更改其jar包名称
			}
			else{
				String pid = getCommandStr.execCommand("ps|grep com.meizu.media.reader|/data/busybox awk '{print $2}'",false).successMsg;  
				Log.i("alitest", "com.meizu.media.reader" + pid);  
			}
		...
		try {
			...
			System.out.println(false);
				...
				if (isMA01 ) {
					if (!getName().equals("test000FristOpen")) {
						if (isP1()) {
							System.out.println("P1");
							this.yunosTest("stop com.meizu.media.reader key");
						}
						else {
							System.out.println("P2");
							this.yunosTest("stop com.meizu.media.reader ");
						}
					}	
			}
...
//		this.exec("ime set com.meizu.flyme.input/com.meizu.input.MzInputService");
	...
	@Steps("阿里启动测试")
	@Expectation("阿里启动测试")   
	public void test000FristOpen() throws AutoException, IOException{
		boolean isMA01 = this.checkUserDebug("m1metal", "test-keys");
		if (isMA01) {
			ShellUtil.exitApp(AppInfo.PACKAGE_READER);
			this.sleep(5000);
			this.startApp(AppInfo.PACKAGE_READER, AppInfo.ACTIVITY_READER);
			this.yunosTest("start com.meizu.media.reader");
			this.sleep(5000);
	...
	@Steps("阿里启动测试2")
	@Expectation("阿里启动测试2")   
	public void test000FristOpen2() throws AutoException, IOException{
		boolean isMA01 = this.checkUserDebug("m1metal", "test-keys");
		if (isMA01) {
			this.startApp(AppInfo.PACKAGE_READER, AppInfo.ACTIVITY_READER);
		}
	...
	@Steps("新手引导页面1.首次打开阅读2.非首次打开阅读")
	@Expectation("1.弹出新手引导页面，显示后台配置的可控选择的栏目2.不弹出新手引导页面")   
	public void test001FristOpen() throws AutoException{
		this.setFloatVisibility(false);
//		this.sendWIFI("MZ-MEIZU", "20runMZ15");
		
		ShellUtil.clearCache(AppInfo.PACKAGE_READER);
		this.startApp(AppInfo.PACKAGE_READER, AppInfo.ACTIVITY_READER);//首次打开
//		this.sleep(5000);
		assertTrue("没有弹出新手引导页面", waitForElement(ReaderUtil.HOME_PAGE, 5000));
		this.assertMoreText("娱乐", "天下事", "科技", "体育", "生活", "财经", "游戏", "汽车", "视觉");
		this.clickByText("确定");
		this.exitReader();;
		this.startApp(AppInfo.PACKAGE_READER, AppInfo.ACTIVITY_READER);//非首次打开
		assertTrue("没有进入阅读", waitForElement(ReaderUtil.READER_CENTER, 5000));
		
	}
	
	@Priority(Priority.P1)
	@Steps("完成新手引导1.选择多个栏目进入首页")
	@Expectation("1.首页显示默认栏目精选以及已订阅栏目")   
	public void test002EnterReader() throws AutoException, RemoteException, IOException{
		...
		//获取引导页选中的分类，存放集合pitchOnCategory
		pitchOnCategory =new ArrayList();
		List<Element> gategory = this.findElements(By.byId(ReaderUtil.GATEGORY_ICON));
		System.out.println(gategory.size());
		for (int i = 0; i < gategory.size(); i++) {
			if (gategory.get(i).isChecked()) {
				pitchOnCategory.add(gategory.get(i).toDown(By.byId(ReaderUtil.GATEGORY_TEXT)).getText());
				System.out.println(gategory.get(i).toDown(By.byId(ReaderUtil.GATEGORY_TEXT)).getText());
		...
		Set homeGategory_after = new HashSet();
		
		//滑动前存放homeGategory_before1
		this.sleep(1000);
		List<Element> homeGategory_before1 = this.findElements(By.byClassName(ReaderUtil.GATEGORY_TAB_TEXT));
		for (int i = 0; i < homeGategory_before1.size(); i++) {
			homeGategory_after.add(homeGategory_before1.get(i).getText());//转移至homeGategory_after
		}
		
		//滑动后存放homeGategory_before2
		this.findElement(ReaderUtil.GATEGORY_TAB).swipeLeft();
		this.sleep(5000);
		List<Element> homeGategory_before2 = this.findElements(By.byClassName(ReaderUtil.GATEGORY_TAB_TEXT));
		for (int i = 0; i < homeGategory_before2.size(); i++) {
			homeGategory_after.add(homeGategory_before2.get(i).getText());//转移至homeGategory_after
		}
		 Iterator it = homeGategory_after.iterator();
		 while (it.hasNext()) {
			System.out.println(it.next());
			
		}
		if(VERSIONS.equals("2.2")){
			pitchOnCategory.add("精选");	//加入精选
			if(ISYUNOS){
				pitchOnCategory.add("推荐");
			}
		}
		else{
		    pitchOnCategory.add("头条");
			if(ISYUNOS){
				pitchOnCategory.add("推荐");
			}
		}
		...
		assertTrue("包含出错", pitchOnCategory.containsAll(homeGategory_after));
		assertTrue("集合大小不一", pitchOnCategory.size() == homeGategory_after.size());	
		
	}
	
	@Priority(Priority.P1)
	@Steps("完成新手引导1.不选栏目直接进入首页")
	@Expectation("1.首页只显示默认的栏目精选")   
	public void test003EnterReader() throws AutoException, RemoteException, IOException{
		this.pub_logout("appadmin111");
		ShellUtil.clearCache(AppInfo.PACKAGE_READER);
		this.startApp(AppInfo.PACKAGE_READER, AppInfo.ACTIVITY_READER);
		this.sleep(1000);
		
		List pitchOnCategory =new ArrayList();
		List<Element> gategory = this.findElements(By.byId(ReaderUtil.GATEGORY_ICON));
		for (int i = 0; i < gategory.size(); i++) {
			if (gategory.get(i).isChecked()) {
				
				gategory.get(i).click();//取消选中
			}
		}
		this.clickByText("确定");
		List<Element> homeGategory = this.findElements(By.byClassName(ReaderUtil.GATEGORY_TAB_TEXT));

		if(VERSIONS.equals("2.2")){
			if(ISYUNOS){
				assertTrue("", homeGategory.size() == 2);
				assertTrue("不是精选+推荐", homeGategory.get(0).getText().equals("精选") &&
						homeGategory.get(1).getText().equals("推荐") );
			}
			else
			{
				assertTrue("集合不止一个", homeGategory.size() == 1);
				assertTrue("不是精选", homeGategory.get(0).getText().equals("精选"));
			}
		}
		else{
			if(ISYUNOS){
				assertTrue("", homeGategory.size() == 2);
				assertTrue("头条+推荐", homeGategory.get(0).getText().equals("头条") &&
						homeGategory.get(1).getText().equals("推荐") );
			}
			else
			{
				assertTrue("集合不止一个", homeGategory.size() == 1);
				assertTrue("不是头条", homeGategory.get(0).getText().equals("头条"));
			}
...
//	     	}
	...
	@Priority(Priority.P2)
	@Steps("1.在收藏夹页面选择多篇文章点击删除按钮")
	@Expectation("1.可成功删除选中的收藏文章")
	public void test064deleteCollect() throws AutoException, RemoteException {
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载  
		this.enterPersonCenterAndClick("收藏");		
		if (isExistByText("请点击登录")) {
			this.click("com.meizu.media.reader:id/empty_image");
		...
		this.deleteCollect();
		this.backHotListFromCollect();
		int number = this.findElement(ReaderUtil.ANDROID_LIST).getChildCount();
		for (int i = 0; i < number - 1; i++) {//收藏
			this.findElementById(ReaderUtil.CONTENT_NAME, i).click();
			this.addCollect();
			this.pressBack();
			this.sleep(200);
		...
		this.findElement(ReaderUtil.RESERVE_TITLE).longClick();
		this.clickByText("全选");
		this.click(ReaderUtil.CENTER_DELETE);
		this.click(ReaderUtil.ANDROID_TEXT1);
		assertTrue("未删除成功", isExistByText("暂无收藏")|| isExistByText("请点击登录"));
...
//	@Priority(Priority.P1)
//	@Steps("首次开启阅读自动离线")
//	@Expectation("通知栏弹出离线下载提示，完成后显示离线文章数量")
//	public void test065AutoOfflineDownload() throws AutoException{
//		
////		this.openNotification();
////		if (isExistByText("离线下载")) {
////			
////			this.leaveNotify();
////			assertTrue(true);
////		}
////		else {
////			boolean isexist = this.searchText(findElement(ReaderUtil.NOTIFICATION_LIST), "离线下载", 60000, true);
////			this.leaveNotify();
////			assertTrue("未找到离线下载", isexist);
////		}
//		//待完成
//		
//		
//	}
	...
	@Priority(Priority.P1)
	@Steps("手动离线1.点击离线2.暂停离线")
	@Expectation("1.点击后开始离线，并线显示状态'正在离线'和离线进度2.点击后暂停离线，并显示状态'已暂停'和离线进度")
	public void test066ManualOfflineDownload() throws AutoException, IOException{
		boolean isMA01 = this.checkUserDebug("m1metal", "test-keys");
		...
		this.startApp(AppInfo.PACKAGE_READER, AppInfo.ACTIVITY_READER);
		this.clickByText("确定");
		this.enterPersonCenterAndClick("离线下载");
		assertTrue("未显示状态'正在离线'和离线进度", isExistByTextContains("正在下载") && isExistByTextContains("%"));
		
		this.clickByText("离线下载");
		assertTrue("未显示状态'正在离线'和离线进度", isExistByTextContains("已暂停", TIME) );
		}
	}
	...
	@Priority(Priority.P2)
	@Steps("点击设置")
	@Expectation("进入应用设置页面")
	public void test067ClickSettings() throws AutoException{
		if(this.findElementByText("选择你喜欢的分类").exists()){
			this.clickByText("确定");
		}
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载  
		this.enterPersonCenterAndClick("设置");
		assertTrue("未进入设置", isExistByText("设置"));
	}
	...
	@Priority(Priority.P1)
	@Steps("进入应用设置，检查默认打开的开关")
	@Expectation("自动离线下载与要闻推送按钮默认打开")
	public void test068ClickSettings() throws AutoException{
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载  
		this.enterPersonCenterAndClick("设置");
		
		Element dowdload = this.findElement(By.byText("无线网络下自动离线下载")).toRight(By.byId(ReaderUtil.SETTING_SWITCH));
		Element push = this.findElement(By.byText("要闻推送")).toRight(By.byId(ReaderUtil.SETTING_SWITCH));
		
		assertTrue("离线下载未关闭", !dowdload.isChecked());
		assertTrue("要闻推送未打开", push.isChecked());
	}
	
	
	@Priority(Priority.P1)
	@Steps("进入意见反馈页面")
	@Expectation("跳转至系统用户反馈页面")
	public void test069TextFeedback() throws AutoException, RemoteException {
		if (!isLoginAccount()) {
			this.pub_login("testtool", "appadmin111");
		}
		this.exitApp(AppInfo.PACKAGE_ACCOUNT);
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载  
		this.enterPersonCenterAndClick("设置");
		this.clickByText("意见反馈");
		if(isExistById(ReaderUtil.SETTING_FEEDBACK_SUGGESTION)){
			this.setTextById(ReaderUtil.SETTING_FEEDBACK_SUGGESTION, "Test1234");
			this.setTextById(ReaderUtil.SETTING_FEEDBACK_CONTACT, "Test");
			this.sleep(1000);
			assertTrue("未填写成功1", isExistByText("Test"));
			this.clickByText("确定");
//		assertTrue("未提交成功", this.logParsing("", "感谢反馈,我们将尽快处理"));
			assertTrue("未提交成功2", !isExistByText("Test1234"));
			assertTrue("未填写成功3", isExistByText("Test"));
		}
		else{
			assertTrue("1", isExistByText("资讯") && isExistByPackageName("com.meizu.feedback"));
	...
	@Priority(Priority.P2)
	@Steps("进入意见反馈页面点击发送图片")
	@Expectation("转跳至系统自带邮箱页面")
	public void test070ImageFeedback() throws AutoException, RemoteException {
		if (!isLoginAccount()) {
			this.pub_login("testtool", "appadmin111");
		}
		this.exitApp(AppInfo.PACKAGE_ACCOUNT);
		...
		this.clickByText("意见反馈");
		if(isExistById(ReaderUtil.SETTING_FEEDBACK_SUGGESTION)){
			this.clickByText("图片反馈");
			assertTrue("未跳转至邮箱", isExistById(ReaderUtil.ANDROID_CONTENT, 20000));
		}
		else{

			this.click(ReaderUtil.FEEDBACK_RADIO);
			this.setTextById(ReaderUtil.FEEDBACK_TEXT, "this is test ! ! ! ! !");
			this.click(ReaderUtil.FEEDBACK_PHOTO);
			this.takePhoto();
			this.setTextById(ReaderUtil.FEEDBACK_ADDRESS, "987654321");
			this.clickByText("提交");
			assertTrue("2", isExistByText("历史反馈"));
		}
	...
	@Priority(Priority.P2)
	@Steps("添加一个桌小插件")
	@Expectation("可成功添加一个4*2的默认桌面小插件")
	public void test071AddLauncherADT() throws AutoException{
		this.pressHome();
		this.pressHome();
		
		int page = this.findElement(By.byId(ReaderUtil.LAUNCHER_PAGE)).findElement(By.byIndex(0)).getChildCount();
		System.out.println("page:" + page);
		Element	last_page = null;
		if (page == 2) {
			last_page = this.findElement(By.byId(ReaderUtil.LAUNCHER_PAGE)).findElement(By.byIndex(0).index(0));
		}
		else {
			last_page = this.findElement(By.byId(ReaderUtil.LAUNCHER_PAGE)).findElement(By.byIndex(0).index(page - 1));
		}
		last_page.click();//点击桌面
		
		this.addLauncherWidget(last_page, "资讯", page);
		
		assertTrue("插件未添加成功", isExistById(ReaderUtil.LAUNCHER_WIDGET_READER, 30000));
	...
	@Priority(Priority.P2)
	@Steps("1.点击桌面小插件列表中的文章2.进入文章后点击返回按钮")
	@Expectation("1.进入相应文章详情页面2.返回应用内的热门页面")
	public void test072SwipeLauncherADT() throws AutoException{
		this.pressHome();
		this.pressHome();
		
		int page = this.findElement(By.byId(ReaderUtil.LAUNCHER_PAGE)).findElement(By.byIndex(0)).getChildCount();//桌面的页数
		System.out.println("page:" + page);
		Element	last_page = null;
		if (page == 2) {//TAMA 两个页面的时候index分布反人类
			last_page = this.findElement(By.byId(ReaderUtil.LAUNCHER_PAGE)).findElement(By.byIndex(0).index(0));
		}
		else {
			last_page = this.findElement(By.byId(ReaderUtil.LAUNCHER_PAGE)).findElement(By.byIndex(0).index(page - 1));
		...
		last_page.click();//定位至最后一页桌面
		if (!isExistById(ReaderUtil.LAUNCHER_WIDGET_READER, 5000)) {//没有插件则添加插件
			this.addLauncherWidget(last_page, "资讯", page);
		}
		
		this.isExistById(ReaderUtil.LAUNCHER_WIDGET_READER, 10000);
		String paper_title = this.getTextById(ReaderUtil.RESERVE_TITLE, 0);
		this.click(ReaderUtil.RESERVE_TITLE);
		assertTrue("未跳转文章", this.isExistById(ReaderUtil.PAPER_COMMENT_EDIT, TIME));
		assertTrue("未跳转点击文章", isExistByDescContains(paper_title, TIME));
		
		this.click(ReaderUtil.PAPER_PICTURE_BACK);//验证返回按钮
		assertTrue("未返回至热门页面", isExistById(ReaderUtil.HOME_IMAGE, TIME));
	...
	@Priority(Priority.P1)
	@Steps("1.点击文章内的超链接")
	@Expectation("1.可通过内置浏览器打开超链接所带url")
	public void test073EnterURL() throws AutoException{
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载  
		this.enterPaper();
		this.locationComment();
		this.sleep(1000);
		this.swipeDown();
		this.swipeDown();
		this.findElement(By.byDescriptionContains("查看原文")).click();
		assertTrue("未打开超链接", isExistById(ReaderUtil.PAPER_SHARE, TIME));
	...
	@Priority(Priority.P2)
	@Steps("1.点击内置网页浏览器顶部关闭按钮")
	@Expectation("1.关闭网页浏览器返回文章详情页面")
	public void test074CloseURL() throws AutoException{
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载  
		this.enterPaper();
		this.locationComment();
		this.sleep(1000);
		...
		this.swipeDown();
		this.findElement(By.byDescriptionContains("查看原文")).click();
		this.isExistByText("关闭", TIME);
		this.clickByText("关闭");
		assertTrue("未返回文章详情页面", isExistById(ReaderUtil.PAPER_COMMENT_EDIT));
	
	}
	
	
	@Priority(Priority.P1)
	@Steps("1.点击样式菜单中的字体大小切换选项")
	@Expectation("1.正文字体大小正确切换至所选标准")
	public void test075ChangeTextSize() throws AutoException{
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载  
		this.enterHotList();
		
		this.click(ReaderUtil.CONTENT_NAME);
		this.isExistById(ReaderUtil.PAPER_COMMENT_EDIT, TIME);
		
	    Element title = this.findElement(By.byClassName("android.webkit.WebView")).findElement(By.byIndex(0)).
	    		findElement(By.byIndex(0));//标题
	    
	    System.out.println(title.getClassName());
	    System.out.println(title.getChildCount());
	    
		
		this.clickMore("小");//小字体
		this.pressBack();
		this.sleep(2000);
		int small_size = title.getBounds().bottom - 
				title.getBounds().top;//小字体的边界值差
		
		this.clickMore("标准");//标准字体
		this.pressBack();
		this.sleep(2000);
		int criterion_size = title.getBounds().bottom - 
				title.getBounds().top;//标准字体的边界值差
		
		this.clickMore("大");//标准字体
		this.pressBack();
		this.sleep(2000);
		int big_size = title.getBounds().bottom - 
				title.getBounds().top;//大字体的边界值差
		
		System.out.println(small_size);
		System.out.println(criterion_size);
		System.out.println(big_size);
		assertTrue("1", small_size < criterion_size);
		assertTrue("2", criterion_size < big_size);
	}
	
	
	@Priority(Priority.P2)
	@Steps("滑动插件检查插件加载的文章数目")
	@Expectation("小插件最多加载16条文章")
	public void test076LoadADT() throws AutoException{
		this.pressHome();
		this.pressHome();
		...
		this.isExistById(ReaderUtil.LAUNCHER_WIDGET_READER, 10000);
		int i = 0 ;
		Set set_titles = new HashSet();
		while (!isExistByText("更多文章") && i < 30) {//滑动插件内容到底部
			List<Element> list_titles = this.findElements(By.byId(ReaderUtil.RESERVE_TITLE));
			for (int j = 0; j < list_titles.size(); j++) {
				set_titles.add(list_titles.get(j).getText());
			}
			this.swipeADT();
			i++;
		}
		
		assertTrue(set_titles.size() <= 16);
	}
	...
	@Steps("点击更多文章")
	@Expectation("进入首页")
	public void test077ClickADTMore() throws AutoException{
		this.pressHome();
		this.pressHome();
		...
		this.clickByText("更多文章");
		assertTrue("未进入首页", isExistById(ReaderUtil.HOME_IMG, TIME));
	}
	
	
	@Priority(Priority.P1)
	@Steps("个人数据拉取1.系统账户已登录2.应用内入口登录账户")
	@Expectation("可成功拉取用户数据（包含回复、评论、收藏夹）")
	public void test078PersonalDataGet() throws RemoteException, AutoException, IOException{
		//登陆账户
		boolean isMA01 = this.checkUserDebug("m1metal", "test-keys");
	    ...
	    this.pub_login("testbackup", "appadmin111");
		...
		if (isExistByText("请点击登录")) {
			this.pressBack();
			this.sleep(1000);
			if(isExistByText("未登录")){
				this.clickByText("未登录");
			}
			this.sleep(3000);
			this.clickByText("消息");
		}
		assertTrue("标题未显示", isExistById(ReaderUtil.CENTER_MESSAGE_REPLY, TIME));
		this.clickByText("我的评论");
		if (isExistByText("请点击登录")) {
			this.click("com.meizu.media.reader:id/empty_image");
		}
	    ...
	    this.pressBack();
	    this.clickByText("收藏");
	    assertTrue("标题未显示", isExistById(ReaderUtil.RESERVE_TITLE) || isExistByText("暂无收藏"));
	    this.pub_logout("appadmin111");
		}
	}
	...
	@Priority(Priority.P1)
	@Steps("点击样式菜单中的夜间模式切换按钮")
	@Expectation("可正常切换或关闭夜间模式风格")
	public void test079NightMode() throws AutoException{
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载   
		this.enterPaper();
		this.openNightMode(true);
		this.sleep(2000);
		String color = this.getRectGRB(ReaderUtil.PAPER_MORE);
		assertTrue("夜间模式切换失败", "292d36".equals(color));
		this.openNightMode(false);
		this.sleep(2000);
		String color_after = this.getRectGRB(ReaderUtil.PAPER_MORE);
		assertTrue("日间模式切换失败", "ffffff".equals(color_after));
		
	}


	@Priority(Priority.P2)
	@Steps("查看首页和普通栏目列表文章标题下的标签")
	@Expectation("文章标题下依次显示：来源、“xx条评论”（或“xx人看过”）")
	public void test080HomePageView() throws AutoException{
		this.waitForElement(ReaderUtil.HOME_IMG, TIME);//等待轮播banner加载
		String paper_local = this.getTextById(ReaderUtil.CONTENT_SITE);
		assertTrue("1", ReaderUtil.PAPER_LOCAL.contains(paper_local));
		String paper_state = this.getTextById(ReaderUtil.CONTENT_COMMENT);
		assertTrue("2", paper_state.endsWith(ReaderUtil.PAPER_STATE_1)
				|| paper_state.endsWith(ReaderUtil.PAPER_STATE_2));
	}
